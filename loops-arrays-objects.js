...
for (let i = 0; i < salaryDetails.length; i++) {
  console.log(
    `Beste ${salaryDetails[i].name}, je salaris van €${salaryDetails[i].salary},- is zojuist overgemaakt. Val me nu alsjeblieft niet meer lastig.`,
  );
}
...
for (let i = 0; i < vacationDays.length; i++) {
  let vacation =
    vacationDays[i].totalVacationDays - vacationDays[i].usedVacationDays;
  console.log(
    `${salaryDetails[i].name} heeft nog ${vacation} vakantiedagen over.`,
  );
}
...
for (let i = 0; i < employeesInTraining.length; i++) {
  if (employeesInTraining[i].month === "Januari") {
    console.log(
      `${employeesInTraining[i].name}: ${employeesInTraining[i].training}`,
    );
  }
}
...
for (let i = 0; i < scores.length; i++) {
  if (scores[i].salaryIncrease === null) {
    ...
    } else if (scores[i].score > 69 && scores[i].score < 90) {
      scores[i].salaryIncrease = "3%";
    } else if (scores[i].score > 89 && scores[i].score < 99) {
...
for (let i = 0; i < employees.length; i++) {
  employees[i].email =
    `${employees[i].firstName}.${employees[i].lastName}@loop-it-solutions.nl`;
